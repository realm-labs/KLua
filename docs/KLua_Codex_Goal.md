...
- Partial `klua-stdlib` support with base, math, string, table, utf8, package, and coroutine library installers covered by focused Lua-source tests, including Lua-backed coroutine yield/resume, protected `pcall`/`xpcall` yield continuation, wrap/close/isyieldable behavior, main/normal coroutine status reporting, coroutine thread type reporting, and host/native yield-boundary checks.
