...
| --- | --- | --- | --- |
| 1 | M20 IO seek boundary audit | Audit `liolib.c:f_seek` across absent/nil/default arguments, `set`/`cur`/`end`, negative and beyond-end positions, invalid options, integer coercion, closed and non-seekable handles, result tuple arity, and position reporting. Close one bounded missing family or retire stale wording, then run focused and full verification. | One coherent IO seek commit; keep platform-specific large-file limits separate. |
