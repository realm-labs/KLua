...
| --- | --- | --- | --- |
| 1 | M20 `string.find` plain-search/index campaign | Audit `lstrlib.c:str_find_aux`, `posrelatI`, `nospecials`, and the length-bounded plain-search helper. Cover raw NUL/control/high/UTF-8 bytes, empty and overlapping needles, magic bytes with true/false plain flags, positive/zero/negative/extreme starts, byte-index results, out-of-range parser bypasses, numeric coercion, ignored extras, result shape, and source-ordered diagnostics. Exit with any fixes, a focused plain-versus-pattern matrix, gap update, and full verification. | One coherent plain-search commit with fixes if needed, tests, and gap-record update. |
