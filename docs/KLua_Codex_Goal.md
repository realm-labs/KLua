...
| --- | --- | --- | --- |
| 1 | M20 OS date-format boundary audit | Audit `loslib.c:os_date`, `checkoption`, and the C `strftime` conversion whitelist for empty formats, `!`/`*t` selection, embedded NULs, invalid and alternate modifiers, buffer-growth failure, extreme timestamps, result arity, and validation order. Keep timezone/locale mutations isolated and restore host state, then run focused and full verification. | One coherent OS date-format commit; document JVM formatter substitutions explicitly. |
