...
- Known Lua 5.5 language gaps include full cross-scope `goto`/label semantics, full lexical local/upvalue `_ENV` name-lowering semantics, and to-be-closed local semantics; `<const>` locals are parsed and enforced, regular and `<const>` named/wildcard `global` declaration scopes, initialized `global` declarations, local-shadowing resolution, explicit default `_ENV` reads and closure-shared default environment assignment, plain function declarations store through resolved bindings and reject const-global targets, and `global function` declarations compile with Lua-style checks, while `<close>` locals are currently rejected before execution.
