...
| --- | --- | --- | --- |
| 1 | M20 `table.concat` range/coercion campaign | Audit `ltablib.c:tconcat`, `addfield`, and `aux_getn` plus Lua's number-to-string conversion. Cover ordinary and table-like receivers, raw NUL/control/high/UTF-8 strings, integer/float spellings, default/custom separators and ranges, empty/reversed/full-width ranges, sparse and invalid values, result sizing, ignored extras, result shape, and source-ordered diagnostics. Exit with any fixes, focused raw-byte/range/error matrices, gap update, and full verification. | One coherent table-concat commit with fixes, tests, and gap-record update. |
