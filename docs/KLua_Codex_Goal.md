...
| --- | --- | --- | --- |
| 1 | M20 IO process/temp-file audit | Audit `liolib.c:io_popen`, `io_pclose`, `io_tmpfile`, `l_checkmodep`, and `lauxlib.c:luaL_execresult`/`luaL_fileresult` for argument ordering, default and platform modes, spawn/open failures, read/write direction, exit result shapes, temporary-file deletion, ignored extra arguments, and close lifecycle. Preserve pure-JVM portability while documenting signal and platform-mode limits, then run focused and full verification. | One coherent process/temp-file commit; keep platform-dependent expectations conditional and deterministic. |
