...
| --- | --- | --- | --- |
| 1 | M20 string byte-primitives campaign | Audit `lstrlib.c:str_len`, `str_sub`, `str_reverse`, `str_rep`, `str_byte`, and `str_char` plus their `posrelatI` and size checks. Cover raw NUL/control/high/UTF-8 bytes, positive/zero/negative/extreme indices, empty and reversed ranges, byte multi-results, character range projection, repetition count/separator/overflow behavior, numeric-string coercion, ignored extras, result shape, and source-ordered diagnostics. Exit with any fixes, a focused cross-entry-point matrix, gap update, and full verification. | One coherent byte-primitives commit with tests and gap-record update. |
