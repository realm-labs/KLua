...
| --- | --- | --- | --- |
| 1 | M20 `math.frexp`/`math.ldexp` decomposition-scaling campaign | Audit `lmathlib.c:math_frexp` and `math_ldexp`, including the checked-integer-to-C-`int` exponent narrowing. Cover both signed zeros, normal and subnormal boundaries, maximum finite values, infinities, NaN, mantissa/exponent subtype and arity, exact round trips, exponent wraparound, underflow/overflow, numeric-string coercion, ignored extras, and left-to-right diagnostics. Exit with any fixes, a focused decomposition/scaling matrix, gap update, and full verification. | One coherent math frexp/ldexp commit with tests and gap-record update. |
