...
| M21 | In progress | The Java/Kotlin ABI, Maven artifacts, executable distributions, stdio DAP host, task-oriented guides, compiled examples, release notes/checklist, canonical JDK 17 baseline, machine-readable performance audit, deterministic checksum bundle, and isolated staged-artifact consumer are locally locked and verified. A secondary supported-runtime smoke remains the active non-publishing frontier; versioning, tagging, signing, pushing, and publication still require explicit authorization. |
...
- Release launcher/process smokes are canonical on JDK 17; a secondary JDK 21 runtime pass has not yet been automated and recorded.
...
| --- | --- | --- | --- | --- |
| 1 | In progress | M21 JDK 17/21 launcher runtime matrix | Use Gradle toolchains to execute the installed compile, debug, and DAP process smokes on canonical JDK 17 and secondary JDK 21, fail if either supported runtime cannot launch the Java 17 artifacts, record the matrix without creating a JDK 21 performance baseline, and keep broader publication actions out of scope. | One to three coherent toolchain-smoke, matrix-verification, and release-guide commits. |
