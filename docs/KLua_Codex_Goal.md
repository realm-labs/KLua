...
| --- | --- | --- | --- |
| 1 | M20 shared registry-table integration | Audit `lstate.c:init_registry`, `lauxlib.c:luaL_getsubtable`/`luaL_requiref`, `loadlib.c:luaopen_package`, and `ldblib.c:db_getregistry`. Add one state-owned registry identity that exposes main-thread/global slots as representable, owns loaded/preload tables across package re-open and global replacement, drives package `LUA_NOENV`, and is returned by `debug.getregistry`; define public low-level API access only if required. Exit with cross-library identity/mutation tests and full verification. | One coherent core/API/stdlib registry commit, or a prerequisite API commit followed by integration, with gap-record updates. |
