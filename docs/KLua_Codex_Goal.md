...
| --- | --- | --- | --- |
| 1 | M20 IO buffering-mode campaign | Audit `file:setvbuf` against `liolib.c:f_setvbuf`, `luaL_checkoption`, `luaL_optinteger`, and the write/flush/seek/close paths. Implement observable `"no"`, `"full"`, and `"line"` output behavior for writable file and process handles within JVM constraints, while preserving ordered validation and source-compatible result shapes. Cover default/explicit sizes, newline-triggered line flush, full-buffer thresholds, explicit flush/seek/close draining, closed/read-only handles, and host failure propagation. Exit with focused IO tests, an updated JVM-deviation note, and full verification. | One coherent IO buffering commit with its semantic matrix and gap-record update; split only if a reusable stream abstraction is independently verified. |
