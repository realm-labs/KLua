...
| --- | --- | --- | --- |
| 1 | M20 `math.min`/`math.max` comparison-identity campaign | Audit `lmathlib.c:math_min`, `math_max`, `lapi.c:lua_compare`, and the `lvm.c`/`ltm.c` less-than path. Cover first-value retention on ties and NaN, mixed integer/float boundaries, unsigned raw string ordering, table/userdata comparison metamethod selection and exact argument order, original result identity and subtype, comparison errors, ignored values only after an earlier error, and non-yieldable library callbacks. Exit with any fixes, a focused comparison/identity matrix, gap update, and full verification. | One coherent math min/max commit with tests and gap-record update. |
