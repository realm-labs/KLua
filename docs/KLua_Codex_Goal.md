...
| --- | --- | --- | --- |
| 1 | M20 IO `setvbuf` argument audit | Audit `liolib.c:f_setvbuf` across open/closed regular and stream handles, required options, optional/default sizes, integer coercion, zero/negative/extreme sizes, result tuple arity, and validation ordering. Close one bounded missing family or precisely retain the known JVM buffering limitation, then run focused and full verification. | One coherent IO buffer-mode commit; do not mix unrelated read/write behavior. |
