...
| --- | --- | --- | --- |
| 1 | M20 `math.sin`/`math.cos`/`math.tan` wrapper campaign | Audit `lmathlib.c:math_sin`, `math_cos`, and `math_tan` plus the configured Lua-number math macros. Cover both signed zeros, representative quadrant and large-finite inputs, infinities, NaN, numeric-string coercion, result subtype/arity, ignored extras, and argument diagnostics, while separating source wrapper semantics from host `libm` approximation differences. Exit with any fixes, a focused trigonometric matrix, gap update, and full verification. | One coherent math trigonometric commit with tests and gap-record update. |
