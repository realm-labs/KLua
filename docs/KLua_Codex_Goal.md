...
| --- | --- | --- | --- |
| 1 | M19 call-frame cold-state audit | Measure the ordinary `CallFrame` footprint, classify yield/debug-only fields against Lua 5.5 `CallInfo` behavior, and either split one coherent cold state lazily with coroutine/debug controls or document why object-layout evidence rejects it. | One focused VM optimization/evidence commit; preserve suspension, hook-transfer, and debugger PC state. |
