...
| --- | --- | --- | --- |
| 1 | M20 string case-byte campaign | Audit `string.lower` and `string.upper` against `lstrlib.c:str_lower`/`str_upper`, `tolower`/`toupper`, byte-length buffer handling, and `os.setlocale` interaction. Preserve every byte not mapped by the active character locale without Unicode expansion or JVM character-count changes. Cover ASCII, embedded NULs, invalid and valid UTF-8 byte sequences, all 256 raw byte values under the default locale, Turkish-sensitive ASCII, Java-installed non-default locales, numeric coercion, ignored extras, result length, and source argument errors. Exit with an explicit locale mapping policy, a focused raw-byte matrix, a narrowed locale/string gap, and full verification. | One coherent string case-mapping commit with tests and gap-record update; a documented default-C-locale limitation is acceptable only where the JVM cannot reproduce byte-locale tables safely. |
