...
- Coroutine runtime yield/resume continuation semantics; the core VM can propagate and resume internal yielded frames, but `coroutine.yield` currently reports outside-coroutine and non-yieldable-boundary errors instead of suspending through the stdlib coroutine API.
