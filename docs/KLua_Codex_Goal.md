...
Lua 5.5's `ldo.c:moveresults` handles zero and one wanted results separately before falling back to general result movement. KLua likewise now uses shared empty and singleton immutable snapshots for zero- and one-value returns, while retaining exact copied lists for multiple results. Snapshot creation remains before the return hook, so debugger writes cannot retroactively alter returned values. A proposed global `LuaStack.slice` specialization was rejected because matched GC showed a 24-byte regression on every native call; native argument snapshots therefore retain their prior representation. Focused tests prove empty reuse and single/multiple snapshot independence after stack mutation.
...
| --- | --- | --- | --- |
| 1 | M19 call-frame stack co-location | Audit removal of the separate per-call `LuaStack` wrapper by co-locating its array and lazy capture state with `CallFrame`, while retaining standalone stack tests and stack-range copying. Preserve open-upvalue identity, suspended-frame visibility, debugger mutation, and dynamic growth. Exit with focused frame/capture tests, matched Lua-call/closure/coroutine GC controls, and the full suite, or reject if inheritance/layout complexity does not yield a clear per-frame reduction. | One focused VM representation/evidence commit; do not add frame pooling or immutable-value unboxing. |
| 2 | M19 post-frame selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
