...
| --- | --- | --- | --- |
| 1 | M20 string pattern capture-state campaign | Audit `start_capture`, `end_capture`, `match_capture`, `get_onecapture`, and `push_captures` for nested string and position captures, rollback during repetition/backtracking, `%0` through `%9` validation, the 32-capture limit, unfinished/invalid capture timing, raw-byte capture values, and result ordering/arity across `find`, `match`, `gmatch`, and `gsub`. Include no-capture whole-match fallback and replacement/callback capture selection without reopening the completed `gsub` byte boundary. Exit with any fixes, a focused capture-state matrix, gap update, and full verification. | One coherent string-pattern capture commit with tests and gap-record update. |
