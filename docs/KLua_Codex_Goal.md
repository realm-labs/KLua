...
| --- | --- | --- | --- |
| 1 | M20 `utf8.codes` iterator audit | Audit `lutf8lib.c:iter_codes`, `iter_aux`, and `utf8_decode` across initial continuations, skipped continuation runs, malformed sequence starts, trailing continuations, strict/lax mode, and arbitrary iterator controls. Close one bounded missing family or retire stale wording, then finish with focused Lua-source cases plus the full suite. | One coherent UTF-8 iterator commit; do not mix unrelated string behavior. |
