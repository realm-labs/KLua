...
| --- | --- | --- | --- |
| 1 | M20 package loadlib-disabled boundary audit | Audit `loadlib.c:ll_loadlib`, `lookforfunc`, and `luaL_fileresult` against KLua's intentional pure-JVM fallback for ordered path/init coercion, embedded-NUL C-string boundaries, ignored extras, exact three-result unsupported tuples, and C/C-root searcher propagation. Keep native-loading unavailability explicit and run focused and full verification. | One coherent package loadlib/searcher commit with tests and gap-record update. |
