...
- Runtime errors preserve structured source-name, line, Lua call-frame metadata, registered host/native call-frame metadata, and readable traceback strings from VM bytecode positions through core execution results, API runtime exceptions, and API coroutine runtime results, and host exceptions can survive as runtime error causes.
