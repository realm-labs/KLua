...
| --- | --- | --- | --- |
| 1 | M19 array-backed Lua stack storage | Audit Lua 5.5's contiguous stack and growth rules, replace per-frame collection storage with an array-backed implementation, and preserve dynamic growth, slice/copy behavior, and open-capture synchronization. Exit with focused stack/capture tests, the full suite, and matched Lua-call, recursive-call, closure, and coroutine evidence, or reject if the representation is not a clear allocation win. | One focused VM representation/evidence commit; do not combine immutable value boxing or frame pooling. |
| 2 | M19 post-stack selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
