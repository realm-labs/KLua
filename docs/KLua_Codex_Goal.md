...
| --- | --- | --- | --- |
| 1 | M20 `string.format` numeric conversion campaign | Audit `lstrlib.c:scanformat`, `getformat`, `addlenmod`, and every integer/float branch of `str_format`. Cover signed and unsigned integer projection for `d`/`i`/`u`/`o`/`x`/`X`, flag-width-precision interactions and limits, alternate forms, integer signed boundaries, floating signed zero/infinities/NaN, `a`/`A`/`e`/`E`/`f`/`g`/`G`, numeric-string acceptance/rejection, ignored extras, left-to-right diagnostics, and host-locale normalization boundaries. Exit with any fixes, a focused numeric-format matrix, gap update, and full verification. | One coherent numeric-format commit with tests and gap-record update. |
