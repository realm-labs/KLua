...
| --- | --- | --- | --- |
| 1 | M20 `string` balanced-pattern byte/reachability campaign | Audit `lstrlib.c:matchbalance` and the `%b` branch of `match`. Cover nested and adjacent balances, identical delimiters, NUL and high-byte delimiters/subjects, missing delimiters, start-position and failed-prefix reachability, captures/backtracking/repetition composition, and result/progress behavior across `find`, `match`, `gmatch`, and `gsub`. Exit with any fixes, a focused cross-consumer `%b` matrix, gap update, and full verification. | One coherent balanced-pattern commit with tests and gap-record update. |
