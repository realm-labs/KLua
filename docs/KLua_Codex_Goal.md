...
- Known Lua 5.5 language gaps include full cross-scope `goto`/label semantics and to-be-closed local semantics; `<const>` locals are parsed and enforced, statically nil/false `<close>` locals compile as no-op close values while non-false close locals are still rejected before execution, regular and `<const>` named/wildcard `global` declaration scopes, initialized `global` declarations, local-shadowing resolution, lexical `_ENV` lowering and closure-shared default environment assignment, plain function declarations store through resolved bindings and reject const-global targets, and `global function` declarations compile with Lua-style checks.
