...
| --- | --- | --- | --- |
| 1 | M20 IO write-result campaign | Audit `io.write` and `file:write` against `liolib.c:g_write`, `lua_numbertocstring`, `fwrite`, and `luaL_fileresult`. Introduce a write primitive that can report accepted bytes so a failure includes bytes written from the failing argument instead of only prior complete arguments. Preserve raw string bytes, numeric formatting, validation order, handle return identity, and buffering behavior. Cover empty/multiple arguments, embedded NULs, integer/float values, read-only handles, deterministic zero/partial/full failing sinks, cumulative counters, four-result failures, flush/close aftermath, and default-output routing. Exit with focused write tests, removal or narrowing of the partial-count gap, and full verification. | One IO write-semantics commit, with a second internal-sink abstraction commit only if independently tested and reusable. |
