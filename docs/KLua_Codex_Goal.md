...
| --- | --- | --- | --- |
| 1 | M19 fixed-arity Lua metamethod arguments | Follow Lua 5.5's `ltm.c:luaT_callTMres`/`luaT_callTM` stack placement by entering Lua closure metamethods from their fixed two- or three-value arguments without first allocating Kotlin `listOf` object arrays and wrappers. Preserve argument order, `__newindex`'s third value, callable metamethod chains, native-function snapshots, call-site metadata, and error/yield boundaries. Exit with index/newindex/operator/length/callable behavior tests, matched function-valued `__index` plus native/ordinary controls, JFR removal of the selected entry snapshot, and the full suite, or reject if overload complexity spreads outside the metamethod boundary. | One focused metamethod-entry/evidence commit; do not combine result transport, frame pooling, or numeric unboxing. |
| 2 | M19 post-argument selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
