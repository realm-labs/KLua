...
| --- | --- | --- | --- |
| 1 | M20 string-format byte-boundary campaign | Audit `string.format` against `lstrlib.c:str_format`, `getformat`, `addquoted`, `addliteral`, `luaL_addvalue`, and the C formatting helpers. Make width/precision and `%s` truncation operate on Lua bytes rather than JVM characters while preserving embedded NULs where the source uses explicit lengths; keep `%q`, `%c`, integer, float, pointer, and invalid-option behavior source-shaped. Cover raw bytes above 0x7f, multibyte source text, embedded NULs, zero/limited precision, width/flags, numeric coercion and extremes, quoting escapes, ignored extras, missing arguments, maximum specifier length, result bytes, and error ordering. Exit with a focused byte matrix, a narrowed string-fidelity gap, and full verification. | One coherent string-format semantics commit with tests and gap-record update; split only if a reusable byte-buffer formatter is independently verified. |
