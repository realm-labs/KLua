...
| --- | --- | --- | --- |
| 1 | M19 lazy stack capture storage | Compare KLua's per-frame capture map with Lua 5.5 open-upvalue creation/closing, then allocate capture storage only when `CLOSURE` first captures a local. Exit with closure correctness plus matched Lua-call and closure-control GC/JFR evidence. | One focused VM optimization/evidence commit; preserve shared-upvalue identity and close behavior. |
| 2 | M19 post-capture selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
