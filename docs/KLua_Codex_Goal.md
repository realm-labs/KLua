...
- Initial `klua-debug` source-line breakpoint manager for setting, replacing, source-wide replacement, clearing, listing, and enabled-hit lookup by source ID and line, public Lua stack-frame and local-variable debugger views with stable value summaries, and a small debug controller for pause, breakpoint, conditional-breakpoint, and step stop decisions.
