...
| Locals, upvalues, and globals are inspectable. | Partial | Live stopped frames now populate locals and captured upvalues; globals are still only a detached view type. |
...
The closure audit above closes M13, M14, and M17 against their success criteria. The first M15 vertical slice now connects breakpoint and step decisions to a real stopped VM context with live locals/upvalues. M15 still needs globals, selected-frame evaluation, coroutine thread/session policy, and an explicit disabled fast path before M16 consumes that context. M18 still needs a production-safe library-access policy before performance work begins.

Use this work-package order:

| Order | Work package | Outcome and exit criteria | Expected commit shape |
| --- | --- | --- | --- |
| 1 | M15 live scopes and evaluation | Populate globals for stopped frames and add restricted expression-only evaluation against the selected live frame. Exit when tests read locals/upvalues/globals in one expression without mutating the stopped program. | One to three commits separated by frame-environment plumbing and evaluator behavior if needed. |
| 2 | M15 coroutine and fast-path closure | Define live debug-session ownership for multiple Lua coroutines and add a configuration/fast-mode test proving the observer path is absent or cheap when debugging is disabled. | One or two runtime/debug integration commits. |
...
| 4 | M18 production-safe access policy | Add an explicit production-safe configuration/preset so OS/file libraries require opt-in, and verify that selective host exposure remains intact. | One or two API/stdlib behavior commits with Java and Kotlin coverage. |
| 5 | M19 benchmark baselines | Replace the dummy benchmark with the first parse/compile and VM execution baselines before optimizing runtime paths. | One benchmark-baseline commit, followed by evidence-backed optimization packages only. |
