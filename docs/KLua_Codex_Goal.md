...
- Broader coroutine runtime hardening, including additional nested coroutine edge cases and Lua 5.5 conformance coverage beyond the current Lua-backed, protected-call, index/newindex read/write-opcode, and arithmetic/bitwise/unary/length metamethod yield/resume paths. Yield completion for comparison and concatenation metamethods remains open.
...
| --- | --- | --- | --- |
| 1 | M20 yieldable comparison metamethod completion | Follow Lua 5.5's `luaV_finishOp` completion for `EQ`, `LT`, and `LE`: permit Lua and explicitly yieldable native metamethods to suspend from Lua opcodes, convert the resumed first result with Lua truthiness, and preserve the comparison opcode's branch/inversion semantics. Cover zero/one/multiple results, both metamethod operand selections, closure/native/callable paths, repeated yields, errors, and native-library non-yieldable boundaries. | One focused VM continuation/conformance commit; exclude iterative concatenation progress state. |
| 2 | M20 yieldable concatenation progress | Follow Lua 5.5's iterative `CONCAT` continuation state, preserving remaining operand order and partial reductions across one or multiple metamethod yields. Cover closure/native/callable paths, repeated yields, errors, and native-library non-yieldable boundaries. | One focused VM continuation/conformance commit after comparison; do not combine with unrelated operator work. |
