...
| --- | --- | --- | --- |
| 1 | M20 table construction campaign | Audit `ltablib.c:tcreate` and `tpack` plus integer conversion and table field insertion. Cover zero/negative/full-width size hints, optional and ignored arguments, numeric-string coercion, range/diagnostic order, all Lua value kinds including nil holes, exact `n` metadata, independent result identity, result shape, and source-ordered failures. Exit with any fixes, focused create/pack matrices, gap update, and full verification. | One coherent table-construction commit with fixes, tests, and gap-record update. |
