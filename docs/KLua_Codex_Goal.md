...
The closure audit above closes M13 through M18 against their success criteria. M19 is now the earliest open milestone: benchmark baselines must replace the dummy benchmark before runtime optimization begins.

Use this work-package order:

| Order | Work package | Outcome and exit criteria | Expected commit shape |
| --- | --- | --- | --- |
| 1 | M19 benchmark baselines | Replace the dummy benchmark with the first parse/compile and VM execution baselines before optimizing runtime paths. | One benchmark-baseline commit, followed by evidence-backed optimization packages only. |
