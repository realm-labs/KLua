...
### M20 Coroutine-Backed Registry Main-Thread Checkpoint
...
KLua already had one stable `CoroutineRuntime.main` handle used by `coroutine.running`, status, close, yieldability, and debug targeting. Opening the coroutine library now installs that actual handle in registry slot 3, and a repeated open retrieves the existing handle and reuses its runtime instead of manufacturing a replacement. Focused coverage proves identity with `coroutine.running`, thread type, main boolean, two-result arity, persistence across coroutine/package re-open, and isolation between states. The remaining timing gap is architectural: slot 3 is nil until the coroutine library is opened, whereas Lua creates the main-thread identity with the state even when the coroutine library is not selected. Focused coroutine/registry and complete repository tests pass.

Use this work-package order:

| Order | Work package | Outcome and exit criteria | Expected commit shape |
| --- | --- | --- | --- |
| 1 | M20 state-born main-thread ownership | Move ownership of the real main-thread handle from coroutine-library initialization to `LuaState` construction, then bind `CoroutineRuntime` to that same handle. Preserve registry slot 3 before any library opens and through library selection/re-open without creating a placeholder disconnected from status/debug/coroutine state. Exit with low-level pre-library slot tests, debug-without-coroutine behavior, standard coroutine identity/lifecycle tests, and full verification. | One coherent API/coroutine ownership commit with tests and gap-record update. |
