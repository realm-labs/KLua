...
| --- | --- | --- | --- |
| 1 | M20 indirect call-name inference audit | Audit the remaining `debug.getinfo(..., "n")` gap against `ldebug.c:basicgetobjname`/`getobjname` and the compiler opcodes that feed them. Select and close one bounded missing inference family, or document verified parity if the recorded gap is stale. Exit with focused compiler/Lua-source cases, full verification, and an updated gap snapshot. | One coherent conformance commit; do not mix unrelated debug behavior. |
