...
| --- | --- | --- | --- |
| 1 | M20 `string.pack`/`unpack` variable-string layout campaign | Audit the `Kchar`, `Kstring`, and `Kzstr` branches of `lstrlib.c:str_pack`, `str_unpack`, and `str_packsize` plus their shared size/position checks. Cover raw bytes and embedded NUL, `c[0]` and fixed padding, every `s[n]` prefix width from 1 through 16, endian and wide-length validation, `z` termination, alignment and `X`, initial/next positions, ignored extras, result shape, length/result overflow, short data, and source-ordered diagnostics. Exit with any fixes, a focused matrix, gap update, and full verification. | One coherent variable-string layout commit with tests and gap-record update. |
