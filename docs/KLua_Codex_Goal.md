...
| --- | --- | --- | --- |
| 1 | M20 math surface-and-constants closure campaign | Audit `lmathlib.c:mathlib`, `randfuncs`, `setrandfunc`, and `luaopen_math`. Cover the exact default Lua 5.5 field set and absence of compatibility-only names, function identity across reopen, independent random-state ownership, `pi`/`huge`/integer-limit values and subtypes, table/global replacement behavior, and mutability/reopen semantics. Reconcile the remaining math gap statement and exit with any fixes, a focused surface/state matrix, gap update, and full verification. | One coherent math surface/closure commit with tests and gap-record update. |
