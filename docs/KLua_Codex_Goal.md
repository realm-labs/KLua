...
| --- | --- | --- | --- |
| 1 | M20 IO numeric scanner audit | Audit `liolib.c:read_number`, `nextc`, `test2`, and `readdigits` against KLua's raw-byte scanner for whitespace/sign handling, decimal and hexadecimal prefixes/fractions/exponents, locale decimal points, malformed-prefix consumption, look-ahead restoration, the 200-byte numeral cap, EOF, and integer/float conversion. Cover direct reads and captured iterators, preserve source byte consumption on failed conversions, then run focused and full verification. | One coherent numeric-read commit; document locale or host-number conversion limits explicitly. |
