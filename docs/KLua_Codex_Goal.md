...
| --- | --- | --- | --- |
| 1 | M19 internal Lua-return transfer | Audit the per-frame `LuaExecutionResult.Returned` and singleton-list boundary selected by JFR. Follow Lua 5.5's `OP_RETURN`/`luaD_poscall`/`moveresults` behavior for fixed and open result counts, preserve return-hook snapshot timing and suspension/debug visibility, and avoid changing the public/native/coroutine result API. Exit with zero/one/multiple/open-result and return-hook tests, matched Lua-call/closure/vararg/host/coroutine controls, and the full suite, or reject if the wrapper cannot be removed without semantic coupling. | One focused internal return-path commit; do not combine value unboxing or public API redesign. |
| 2 | M19 post-return selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
