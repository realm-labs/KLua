...
| --- | --- | --- | --- |
| 1 | M20 `math.atan`/`math.log` argument-and-IEEE campaign | Audit `lmathlib.c:math_atan` and `math_log` plus the configured Lua-number math macros. Cover `atan2` quadrants across both signed zeros, finite axes, infinities, and NaN; default, explicit-nil, and checked second arguments; logarithm domain edges, signed zero, infinities, NaN, and the source's base-2/base-10/general-base branches; result subtype/arity, ignored extras, and left-to-right diagnostics. Exit with any fixes, a focused matrix, gap update, and full verification. | One coherent math atan/log commit with tests and gap-record update. |
