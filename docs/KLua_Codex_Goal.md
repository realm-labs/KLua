...
| --- | --- | --- | --- |
| 1 | M20 `table.sort` partition audit | Audit `ltablib.c:sort`, `auxsort`, `partition`, and comparator calls across default/custom ordering, small and unbalanced partitions, invalid-order detection, table-like receivers, and non-yieldable comparator boundaries. Close one bounded missing family or retire stale wording, then finish with focused Lua-source cases plus the full suite. | One coherent table-conformance commit; do not mix unrelated library behavior. |
