...
| --- | --- | --- | --- |
| 1 | M20 IO open/default-handle campaign | Audit `liolib.c:io_open`, `io_tmpfile`, `getiofile`, `g_iofile`, `io_close`, and `io_type`. Cover every supported mode and invalid mode, C-string filename/mode boundaries, default getter/setter identity and replacement, closed/default/standard handles, named-open and temporary-file failures, ignored extras, result arity, and source-ordered diagnostics. Exit with any fixes, focused handle-lifecycle matrices, gap update, and full verification. | One coherent IO open/default-handle commit with fixes, tests, and gap-record update. |
