...
| --- | --- | --- | --- |
| 1 | M19 zero-based frame metadata | Remove `base`, `returnBase`, and frame-level `expectedResults` after proving every frame owns a zero-based stack, no constructor supplies non-default values, and pending call-result state already owns suspension metadata. Preserve debug transfer indices and all call/result behavior. Exit with focused register/transfer assertions, matched Lua-call/closure/coroutine GC controls, and the full suite, or reject if object alignment yields no reduction. | One focused frame-layout/evidence commit; do not mix result transport or numeric unboxing. |
| 2 | M19 post-layout selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
