...
- Initial `klua-dap` typed session surface for initialize requests, advertised debugger capabilities, `setBreakpoints` source breakpoint replacement, `configurationDone`, pause/continue/step controls backed by the debug controller, stackTrace/scopes/variables adapters over debug frame views, and an expression-evaluation hook.
