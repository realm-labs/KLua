...
| --- | --- | --- | --- |
| 1 | M20 math integer-boundary campaign | Audit `lmathlib.c:math_abs`, `math_floor`, `math_ceil`, `math_fmod`, `math_modf`, `math_toint`, and `math_ult` plus their Lua integer/number conversion helpers. Cover minimum/maximum integers, subtype preservation, signed zero, infinities, NaN, integer and floating zero divisors, numeric strings, ordered argument errors, ignored extras, exact result arity, and unsigned ordering around the sign bit. Exit with any fixes, a focused numeric-boundary matrix, gap update, and full verification. | One coherent math-boundary commit with tests and gap-record update. |
