...
- Known Lua 5.5 language gaps include full cross-scope `goto`/label semantics, full lexical `_ENV` lowering for initialized `global` declarations and `global function` declarations, and to-be-closed local semantics; `<const>` locals are parsed and enforced, regular and `<const>` named/wildcard `global` declaration scopes, initialized `global` declarations, local-shadowing resolution, explicit default `_ENV` reads and closure-shared default environment assignment, ordinary global-name reads and writes through local/upvalue `_ENV`, plain function declarations store through resolved bindings and reject const-global targets, and `global function` declarations compile with Lua-style checks, while `<close>` locals are currently rejected before execution.
