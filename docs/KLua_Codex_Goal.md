...
| --- | --- | --- | --- |
| 1 | M20 `math.asin`/`math.acos`/`math.sqrt` domain-edge campaign | Audit `lmathlib.c:math_asin`, `math_acos`, and `math_sqrt` plus the configured Lua-number math macros. Cover both signed zeros, exact domain boundaries, just-inside and just-outside values, infinities, NaN, numeric-string coercion, result subtype/arity, ignored extras, and argument diagnostics. Exit with any fixes, a focused unary-domain matrix, gap update, and full verification. | One coherent math inverse/sqrt commit with tests and gap-record update. |
