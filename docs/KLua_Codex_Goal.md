...
| --- | --- | --- | --- |
| 1 | M20 IO seek-boundary campaign | Audit `file:seek` against `liolib.c:f_seek`, `luaL_checkoption`, `luaL_optinteger`, and the platform `l_fseek` range. Preserve source argument ordering and C-string whence options while making base-plus-offset overflow deterministic instead of allowing JVM `Long` wraparound. Cover set/cur/end, defaults and numeric coercion, embedded-NUL options, minimum/maximum offsets, unchanged position after failures, buffered-write draining, non-seekable process handles, result arity, and closed handles. Exit with focused seek tests, any required JVM range note, and full verification. | One coherent IO seek semantics commit with tests and gap-record update. |
