...
| --- | --- | --- | --- |
| 1 | M20 `table.unpack` range/result campaign | Audit `ltablib.c:tunpack`, `luaL_len`, unsigned result-count checks, and indexed reads. Cover ordinary, sparse, string, and table-like receivers; default/custom, empty/reversed/full-width ranges; nil preservation; result-count and stack limits; length/index metamethod order; numeric coercion; ignored extras; result shape; and source-ordered diagnostics. Exit with any fixes, focused range/arity/error matrices, gap update, and full verification. | One coherent table-unpack commit with fixes, tests, and gap-record update. |
