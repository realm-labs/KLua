...
| --- | --- | --- | --- |
| 1 | M20 `utf8.offset` boundary audit | Audit `lutf8lib.c:byteoffset` across positive/negative/zero offsets, default and explicit positions, continuation-byte rejection, range errors, and empty/boundary strings. Close one bounded missing family or retire stale wording, then finish with focused Lua-source cases plus the full suite. | One coherent UTF-8 conformance commit; do not mix unrelated string behavior. |
