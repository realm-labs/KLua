...
| --- | --- | --- | --- |
| 1 | M19 closure-owned frame metadata | Follow Lua 5.5's function-slot ownership by deriving each frame's prototype and upvalues from its `LuaClosure` instead of retaining duplicate references. Preserve the exact closure identity exposed to debug APIs and the separately resolved environment fallback. Exit with closure/upvalue/debug-function identity tests, matched Lua-call/closure/coroutine GC controls, and the full suite, or reject if object alignment yields no reduction. | One focused frame-layout/evidence commit; do not combine environment representation or numeric unboxing. |
| 2 | M19 post-closure selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
