...
| --- | --- | --- | --- |
| 1 | M20 IO write/flush campaign | Audit `liolib.c:g_write`, `io_write`, `f_write`, `io_flush`, `f_flush`, and `luaL_fileresult`. Cover raw NUL/high bytes, integer/float formatting boundaries, multiple-value ordering, returned-handle identity, partial writes and buffering, unreadable/unwritable handles, flush sequencing, ignored extras where applicable, source-ordered diagnostics, and exact failure tuples. Exit with any fixes, focused direct/default-handle matrices, gap update, and full verification. | One coherent IO write/flush commit with fixes, tests, and gap-record update. |
