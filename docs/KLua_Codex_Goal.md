...
| --- | --- | --- | --- |
| 1 | M20 IO close/flush result audit | Audit `liolib.c:aux_close`, `f_close`, `io_close`, `f_gc`, `io_noclose`, `aux_flush`, and `io_flush` across regular, standard, process, default, closed, read-only, and write-only handles. Verify result tuple arity, error-vs-result boundaries, handle-state transitions, and `__close` result suppression; close one bounded missing family or retire stale wording, then run focused and full verification. | One coherent IO lifecycle commit; keep unrelated read-format and platform-mode work separate. |
