...
| --- | --- | --- | --- |
| 1 | M20 IO lines ownership audit | Audit `liolib.c:aux_lines`, `f_lines`, `io_lines`, and `io_readline` across default/named/file-handle sources, format capture and validation timing, manual iterator arguments, EOF/error closure, early generic-for close, already-closed handles, and four-result ownership. Close one bounded missing family or retire stale wording, then run focused and full verification. | One coherent IO iterator commit; keep numeric scanner behavior separate. |
