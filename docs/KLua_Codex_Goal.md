...
| --- | --- | --- | --- |
| 1 | M20 package initialization-path audit | Audit `loadlib.c:luaopen_package`, `setpath`, `setprogdir`, and `noenv` for versioned/unversioned environment lookup, `LUA_NOENV`, default-path insertion at empty `;;` fields, executable-directory substitution, host separators, registry table identity, and exported configuration. Define an explicit pure-JVM policy for process environment and executable-directory discovery, then run focused and full verification. | One coherent package initialization commit with tests and gap-record update. |
