...
| --- | --- | --- | --- |
| 1 | M20 IO open/default routing audit | Audit `liolib.c:l_checkmode`, `io_open`, `opencheck`, `g_iofile`, and `getiofile` across absent/nil/numeric filenames, mode grammar, open failures, open/closed file handles, default replacement/getters, and later use of closed defaults. Close one bounded missing family or retire stale wording, then run focused and full verification. | One coherent IO routing commit; keep read formats and platform-specific errno mapping separate. |
