...
- Broader coroutine runtime hardening, including additional nested coroutine edge cases and Lua 5.5 conformance coverage beyond the current Lua-backed, protected-call, and index/newindex read/write-opcode yield/resume paths. Yield completion for arithmetic, length, comparison, concatenation, and unary/bitwise metamethods remains open.
...
| --- | --- | --- | --- |
| 1 | M20 value-producing operator metamethod completion | Follow Lua 5.5's `luaV_finishOp` placement for binary arithmetic/bitwise operations and `UNM`, `BNOT`, and `LEN`. Permit Lua and explicitly yieldable native metamethods to suspend from Lua opcodes and resume with their first result, including nil padding and repeated yields. Preserve operand duplication for unary/length calls, call-site names, error behavior, and native-library non-yieldable boundaries. Exit with an opcode-family/result matrix, full tests, and allocation controls. | One focused VM continuation/conformance commit; exclude comparison truth conversion and iterative concatenation. |
| 2 | M20 remaining result-valued continuation selection | Select comparison truth conversion or iterative concatenation from the remaining `luaV_finishOp` state machines after the value-placement family closes. | One bounded conformance package; do not combine comparison branch semantics with concatenation progress state. |
