...
| --- | --- | --- | --- |
| 1 | M19 vararg/multi-return copy audit | Trace the selected `toMutableList` allocation through VM call, `OP_VARARG`, and result movement; compare behavior-sensitive choices with Lua 5.5; then make one bounded optimization with matched GC/JFR evidence or document why the evidence rejects it. | One focused optimization/evidence commit; preserve nil holes, fixed-result adjustment, and vararg snapshots. |
| 2 | M19 post-vararg selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
