...
| --- | --- | --- | --- |
| 1 | M20 OS string-coercion audit | Audit every `loslib.c` `luaL_checkstring`/`luaL_optstring` path against KLua's shared OS string helper for exact Lua integer/float formatting, locale decimal points, mutation-after-conversion behavior, argument ordering, nil defaults, and rejection of non-string/non-number values across `execute`, `getenv`, `remove`, `rename`, `setlocale`, and `date`. Use test-owned filenames and side-effect-free commands, then run focused and full verification. | One coherent OS coercion commit; separate host lookup outcomes from conversion evidence. |
