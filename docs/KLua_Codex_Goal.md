...
| --- | --- | --- | --- |
| 1 | M19 table-literal capacity hints | Compare KLua table-constructor lowering and `NEW_TABLE` execution with Lua 5.5's array/hash size hints, then pre-size only statically known literal fields or reject the change with matched entity-kernel GC/JFR evidence. | One focused compiler/VM optimization with bytecode and behavior tests; preserve dynamic keyed/list-field evaluation order. |
| 2 | M19 post-table selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
