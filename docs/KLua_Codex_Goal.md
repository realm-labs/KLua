...
| --- | --- | --- | --- |
| 1 | M20 IO read-format campaign | Audit `liolib.c:g_read`, `read_line`, `read_all`, `read_chars`, and `read_number` across `io.read` and `file:read`. Cover zero and full-width byte counts, EOF before/after partial data, raw NUL/high bytes, line terminators, multiple-format partial results, numeric token boundaries and subtype, invalid-format order, and exact result arity. Exit with any fixes, focused direct/default-handle matrices, gap update, and full verification. | One coherent IO read-format commit with fixes, tests, and gap-record update. |
