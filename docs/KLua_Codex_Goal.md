...
| --- | --- | --- | --- |
| 1 | M20 `string.gsub` argument/replacement boundary campaign | Audit `str_gsub`, `add_s`, and `add_value` for source-ordered replacement-type versus limit validation, maximum-count behavior, capture selection, string replacement escape parsing, numeric/string coercion, function/table call arguments and result conversion, nil/false preservation, anchored and zero-length matches, and raw NUL/high-byte replacement data. Include combined-invalid-argument cases so validation order is observable and keep yield behavior at the native library boundary. Exit with any fixes, a focused direct/function/table matrix, gap update, and full verification. | One coherent `gsub` boundary commit with tests and gap-record update. |
