...
| --- | --- | --- | --- |
| 1 | M20 package require-name boundary audit | Audit `loadlib.c:ll_require`, `searcher_Lua`, `searcher_C`, `searcher_Croot`, and `loadfunc` for numeric coercion, embedded-NUL module-name boundaries, loaded/preload key identity, Lua/C filename substitution, open-symbol and ignore-mark construction, loader arguments, diagnostics, and result arity. Keep the Lua-source implementation aligned with the C-string API and run focused and full verification. | One coherent package require/searcher commit with tests and gap-record update. |
