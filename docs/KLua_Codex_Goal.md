...
| --- | --- | --- | --- |
| 1 | M19 call-site metadata consolidation | Preserve the prototype's immutable `CallSiteInfo` object on each frame instead of expanding it into separate name/name-kind references. Exit with traceback/debug-name correctness and matched Lua-call/recursive-call GC evidence, or reject if object alignment yields no reduction. | One focused VM optimization/evidence commit; preserve Lua 5.5-style call-site name reporting. |
| 2 | M19 post-metadata selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
