...
| --- | --- | --- | --- |
| 1 | M20 OS environment lookup audit | Audit `loslib.c:os_getenv` and `luaL_checkstring` for ordered string/number coercion, absent/nil/type errors, embedded-NUL C-string boundaries, unavailable-variable nil, empty values, ignored extras, and one-result arity. Define the JVM policy for invalid host environment names and run focused and full verification. | One coherent OS environment commit with tests and explicit host limitations. |
