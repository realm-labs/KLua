...
| --- | --- | --- | --- |
| 1 | M20 string pattern-class byte campaign | Audit `%a/%c/%d/%g/%l/%p/%s/%u/%w/%x/%z`, complements, bracket classes, and frontiers against `lstrlib.c:match_class`, `matchbracketclass`, and `match`. Make every predicate consume one unsigned Lua byte and share the explicit case-mapping locale policy without JVM Unicode classification. Cover all 256 bytes, complements, embedded NULs, malformed/valid UTF-8, ranges mixed with classes, frontier boundaries, selected Turkish locale behavior, captures/find/match/gmatch/gsub consumers, and invalid pattern errors. Exit with a focused predicate matrix, documented non-ASCII C-locale limits, and full verification. | One coherent string-pattern byte-class commit with tests and gap-record update. |
