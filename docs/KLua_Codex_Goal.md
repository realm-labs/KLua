...
| --- | --- | --- | --- |
| 1 | M20 cross-thread debug state audit | Audit the remaining optional-thread gap against `ldblib.c:getthread` and the `lua_getstack`/`lua_getinfo` state rules. Define a reachable current/suspended/normal/dead/error state matrix, close one bounded missing family or retire stale wording, and finish with focused Lua-source cases plus the full suite. | One coherent debug-conformance commit; do not mix unrelated library behavior. |
