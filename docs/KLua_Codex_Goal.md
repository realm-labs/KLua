...
| --- | --- | --- | --- |
| 1 | M20 OS filesystem-result audit | Audit `loslib.c:os_remove`, `os_rename`, and `os_tmpname` plus `lauxlib.c:luaL_fileresult` for argument conversion/order, success arity, filename-qualified versus unqualified failures, numeric error codes, missing paths, directory and overwrite behavior, temporary-name uniqueness/noncreation, ignored extra arguments, and platform-dependent rename policy. Preserve deterministic portable assertions and document host filesystem limits, then run focused and full verification. | One coherent OS filesystem commit; keep destructive operations inside test-owned temporary directories. |
