...
| --- | --- | --- | --- |
| 1 | M20 IO open-boundary campaign | Audit `io.open`, `io.input`, `io.output`, `io.lines(filename)`, and `io.popen` against `liolib.c:io_open`, `opencheck`, `g_iofile`, `io_lines`, `io_popen`, `l_checkmode`, and `l_checkmodep`. Apply source C-string boundaries to filenames, commands, and modes without losing Lua-side argument ordering; preserve repeated binary extensions and platform-specific popen modes. Cover embedded NULs in every string, default/nil/numeric modes, repeated `b`, update/append/truncate behavior, filename-prefixed failures, default-handle replacement, iterator-owned closing, process read/write modes, ignored extras, and result arity. Exit with focused open/default/popen tests, updated JVM path/process deviations, and full verification. | One coherent IO open-boundary commit with tests and gap-record update. |
