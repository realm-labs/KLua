...
| --- | --- | --- | --- |
| 1 | M20 next conformance campaign selection | Select one explicit remaining subsystem gap after the protected-call performance package. Name the relevant Lua 5.5 source functions and affected KLua entry points, define a bounded case matrix, and update the gap snapshot after verification. | One coherent conformance commit; do not mix unrelated probes. |
| 2 | M19 evidence-selected performance continuation | After the conformance campaign, profile one remaining measured allocation residual that does not require the deferred numeric/value representation rewrite. Exit with an immediate negative control, full correctness, and the 14-control matrix. | One measured optimization or documented rejection package. |
