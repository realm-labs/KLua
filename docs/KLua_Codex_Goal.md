...
| --- | --- | --- | --- |
| 1 | M20 OS locale-category audit | Audit `loslib.c:os_setlocale`, `luaL_optstring`, and `luaL_checkoption` for ordered argument coercion, default and explicit categories, query versus mutation calls, `C`/empty/unsupported locale names, ignored extras, result arity, and restoration after controlled mutations. Define and test the JVM policy for category-specific requests instead of silently treating every category as a process-wide Java locale change, then run focused and full verification. | One coherent OS locale commit with tests and explicit JVM/global-state limitations. |
