...
| --- | --- | --- | --- |
| 1 | M20 string pattern structural-error reachability campaign | Audit `classend`, `matchbalance`, the `%f` branch of `match`, and consumer entry loops for the source rule that malformed `%`, bracket, balanced, frontier, and capture items raise only when matching execution reaches them. Cover failed literal prefixes, alternate search starts, successful prefixes, anchors, plain `find`, out-of-range starts, zero/negative `gsub` limits, lazy `gmatch` iteration, and exact Lua 5.5 diagnostics. Refactor tokenization only as far as needed to preserve lazy source timing without weakening valid-pattern matching. Exit with focused reachability/error matrices, gap update, and full verification. | One coherent pattern error-timing commit with tests and gap-record update. |
