...
| --- | --- | --- | --- |
| 1 | M20 string pattern repetition and empty-match campaign | Audit quantified single-item control flow and consumer progression against `lstrlib.c:match`, `max_expand`, `min_expand`, `str_find_aux`, `gmatch_aux`, and `str_gsub`. Cover zero/one/many matches for `?`, `*`, `+`, and `-`; greedy and minimal backtracking into following literals/classes/captures; anchors and byte-zero data; empty matches at the beginning, between bytes, and at the end; `find`/`match` initial positions; and `gmatch`/`gsub` once-only forward progress. Exit with any semantic fixes, a bounded cross-consumer matrix, gap update, and full verification. | One coherent string-pattern repetition/consumer commit with tests and gap-record update. |
