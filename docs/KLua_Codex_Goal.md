...
| --- | --- | --- | --- |
| 1 | M20 `math.exp`/`math.deg`/`math.rad` range-and-scaling campaign | Audit `lmathlib.c:math_exp`, `math_deg`, and `math_rad` plus the configured Lua-number constants and operation ordering. Cover both signed zeros, underflow/overflow thresholds, infinities, NaN, angle conversion round trips and large-value overflow, numeric-string coercion, result subtype/arity, ignored extras, and argument diagnostics. Exit with any fixes, a focused exponential/scaling matrix, gap update, and full verification. | One coherent math exp/angle commit with tests and gap-record update. |
