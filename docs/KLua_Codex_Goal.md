...
| --- | --- | --- | --- |
| 1 | M19 numeric for-loop boxing audit | Profile `advanceForLoop` across the numeric-loop and vararg controls, compare integer/float step and overflow behavior with Lua 5.5's `OP_FORLOOP`, and either make one bounded optimization with matched evidence or document why the evidence rejects it. | One focused optimization/evidence commit; preserve visible control-variable values and integer overflow termination. |
| 2 | M19 post-loop selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
