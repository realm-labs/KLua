...
| --- | --- | --- | --- |
| 1 | M20 OS execute-result audit | Audit `loslib.c:os_execute`, `l_system`, and `lauxlib.c:luaL_execresult` for absent/nil shell probes, command conversion, inherited IO, zero/nonzero exits, spawn failures, result arity, POSIX signal reporting, interruption, and ignored extra arguments. Preserve pure-JVM embedding safety and document wait-status limits, then run focused and full verification. | One coherent OS process-result commit; keep commands side-effect-free and host-conditional. |
