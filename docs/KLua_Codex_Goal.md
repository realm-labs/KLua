...
- Broader coroutine runtime hardening, including additional nested coroutine edge cases and Lua 5.5 conformance coverage beyond the current Lua-backed, protected-call, and `__index` read-opcode yield/resume paths. Yield completion for `__newindex`, arithmetic, length, comparison, concatenation, and unary/bitwise metamethods remains open.
...
| --- | --- | --- | --- |
| 1 | M20 yieldable `__newindex` completion | Follow Lua 5.5's `luaT_callTM` and `luaV_finishOp` behavior for `SETTABLE`, `SETFIELD`, and environment writes. Permit Lua and explicitly yieldable native `__newindex` functions to suspend when called from Lua code, resume with all results discarded, and preserve table-valued chains, receiver/key/value order, cycle limits, errors, and native-library non-yieldable boundaries. Exit with a Lua-source case matrix for the three KLua write opcodes, closure/native continuations, table chains, repeated yields, and C/native boundary rejection, plus the full suite and allocation controls. | One focused VM continuation/conformance commit; do not mix result-valued operators or reopen the rejected result-allocation optimization. |
| 2 | M20 result-valued metamethod continuation selection | Inspect the remaining `luaV_finishOp` groups and select one coherent family: arithmetic/unary/length value placement, comparison truth conversion, or iterative concatenation. | One bounded conformance package with an explicit opcode/result matrix; do not combine unrelated finish-operation state machines. |
