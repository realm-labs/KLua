...
| --- | --- | --- | --- |
| 1 | M19 fixed-arity empty-vararg storage | Follow Lua 5.5's vararg-frame split and avoid allocating mutable empty storage for non-vararg prototypes. Preserve mutable owned storage for real vararg frames and all debugger/bytecode behavior. Exit with fixed/vararg ownership tests, matched Lua-call and vararg GC controls, and the full suite, or reject if allocation is not reduced without complicating writes. | One focused frame-storage/evidence commit; do not combine general result-list or value-representation changes. |
