...
| --- | --- | --- | --- |
| 1 | M20 IO seek/buffering campaign | Audit `liolib.c:f_seek`, `f_setvbuf`, `aux_flush`, and their interaction with reads, writes, and close. Cover all seek origins, signed/full-width offsets and host narrowing, C-string/invalid options, default and ignored arguments, seekable versus stream handles, every buffer mode and size boundary, drain-before-seek/reconfigure/close ordering, partial drain failures, file-position/result shapes, and source-ordered diagnostics. Exit with any fixes, focused position/buffering matrices, gap update, and full verification. | One coherent IO seek/buffering commit with fixes, tests, and gap-record update. |
