...
- Known debug-library gaps include optional thread arguments for debug functions, full `debug.getuservalue`/`debug.setuservalue` userdata support, and non-table metatables through `debug.setmetatable`.
