...
- Known Lua 5.5 language gaps include full cross-scope `goto`/label semantics, full global declaration indexed-write diagnostics, and to-be-closed local semantics; `<const>` locals are parsed and enforced, regular and `<const>` named/wildcard `global` declaration scopes, initialized `global` declarations, local-shadowing resolution, and `global function` declarations compile with Lua-style checks, while `<close>` locals are currently rejected before execution.
