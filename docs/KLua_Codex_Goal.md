...
| --- | --- | --- | --- |
| 1 | M19 evidence-selected performance continuation | Rerun matched allocation and JFR controls after the completed continuation campaign, then select one bounded residual that does not require the deferred numeric/value representation rewrite. Exit with an immediate negative control, full correctness, and the 14-control matrix. | One measured optimization or documented rejection package. |
| 2 | M20 next conformance campaign selection | Select one explicit remaining subsystem gap after the performance package. Name the relevant Lua 5.5 source functions and affected KLua entry points, define a bounded case matrix, and update the gap snapshot after verification. | One coherent conformance commit; do not mix unrelated probes. |
