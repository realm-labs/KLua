...
| --- | --- | --- | --- |
| 1 | M19 single-result Lua metamethod completion audit | Inspect Lua 5.5's `ltm.c:luaT_callTMres` and `ldo.c` result/hook/yield behavior, then determine whether KLua can transfer one Lua-closure metamethod result without allocating a general `LuaExecutionResult.Returned` plus list. Preserve return-hook ordering, nil for no result, errors, callable/native boundaries, and all suspension behavior. Exit with an explicit result matrix, matched function-valued `__index` plus ordinary/native/coroutine controls, a post-change JFR, and the full suite; reject and document the result if a local specialization cannot preserve those boundaries. | One focused result-transport/evidence commit or one documented rejection; do not combine public result APIs, frame pooling, stack representation, or numeric unboxing. |
| 2 | M19 post-result selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
