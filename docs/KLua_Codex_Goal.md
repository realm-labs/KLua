...
| --- | --- | --- | --- |
| 1 | M20 yieldable concatenation progress | Follow Lua 5.5's `luaV_finishOp`/`luaV_concat` continuation state, preserving the partially reduced value, all remaining operands, operand order, and raw string/number batching across one or multiple metamethod yields. Cover closure/native/callable paths, repeated yields at different reduction points, zero/one/multiple results, errors, and native-library non-yieldable boundaries. Exit with an operand/progress matrix, full tests, and allocation controls. | One focused VM continuation/conformance commit; do not combine with unrelated operator work. |
| 2 | M19 evidence-selected performance continuation | After concatenation closes the known metamethod-yield gap, rerun matched allocation/JFR controls and select one bounded residual that does not require the deferred numeric/value representation rewrite. | One measured optimization or documented rejection package with full correctness and 14-control verification. |
