...
While building that matrix, one deliberately large numeric-loop body exposed a separate compiler limitation before any library call executed: KLua's custom `ABC` instruction currently gives `TEST`, `JMP`, `FOR_TEST`, and `FOR_LOOP` only one signed byte of branch displacement, and `BytecodeWriter.jumpOffset` rejects a source span outside -128 through 127 instructions. The pack matrix was factored into small helpers so this package remains about library semantics, but the compiler defect is now explicit and takes priority in the next bounded campaign.

Use this work-package order:

| Order | Work package | Outcome and exit criteria | Expected commit shape |
| --- | --- | --- | --- |
| 1 | M20 compiler long-branch campaign | Audit Lua 5.5 `lopcodes.h` branch fields plus `lcode.c:fixjump`, jump lists, loop lowering, and KLua's instruction packing/VM decoding/package serialization. Remove the custom -128..127 instruction ceiling for forward and backward `if`, logical, `while`, `repeat`, numeric/generic `for`, and `break` paths while preserving debug lines and dumped-chunk validation. Exit with source-sized boundary tests around the old limit, malformed-bytecode checks, focused compiler/VM/package verification, gap update, and the full suite. | One or two coherent bytecode/compiler commits with tests and gap-record update. |
| 2 | M20 `string.pack`/`unpack` floating-layout campaign | Audit `lstrlib.c:copywithendian` and the `Kfloat`/`Knumber`/`Kdouble` branches of `str_pack`, `str_unpack`, and `str_packsize`. Cover float narrowing and rounding, both signed zeros, normal/subnormal endpoints, infinities, NaN payload/sign boundaries, native/opposite byte order, alignment and `X`, numeric-string coercion, ignored extras, result subtype/arity/position, short data, and source-ordered diagnostics. Exit with any fixes, a focused raw-bit matrix, gap update, and full verification. | One coherent floating-layout commit with tests and gap-record update. |
