...
| --- | --- | --- | --- |
| 1 | M20 IO close-finalization campaign | Audit `file:close`, `io.close`, `__close`, and iterator-owned closing against `liolib.c:aux_close`, `f_close`, `io_fclose`, `io_pclose`, and `f_gc`. Mark nonstandard handles closed before attempting buffer drain and underlying close, ensure every owned resource gets a close attempt, and preserve the first source-shaped failure without leaving a retryable half-closed handle. Cover buffered drain failure, underlying close failure, combined failures, repeated close/use errors, `io.type`, default output, process wait/exit results, `<close>` suppression, iterator exhaustion/early exit, delete-on-close cleanup, and non-closing standard handles. Exit with deterministic failing-sink tests, lifecycle verification, an updated gap note, and full verification. | One coherent IO lifecycle commit with tests and gap-record update. |
