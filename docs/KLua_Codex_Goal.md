...
| --- | --- | --- | --- |
| 1 | M19 metamethod call-site metadata reuse | Reuse immutable `CallSiteInfo` records for the VM's fixed metamethod keys instead of rebuilding the record and stripped name string on every Lua metamethod call. Preserve debug/traceback name and `namewhat = "metamethod"` behavior for every operator, index, new-index, length, comparison, concatenation, and call path. Exit with exact call-site-name tests, matched function-valued `__index` plus ordinary table/Lua-call controls, JFR removal of the selected site, and the full suite, or reject if lookup overhead offsets the allocation removal. | One focused metadata-cache/evidence commit; do not combine metamethod result transport or numeric unboxing. |
| 2 | M19 post-metamethod selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
