...
| --- | --- | --- | --- |
| 1 | M20 OS time-table normalization audit | Audit `loslib.c:os_time`, `getfield`, `getboolfield`, `setallfields`, and `l_checktime` for absent/nil calls, ignored extras, ordered field reads, required/default fields, exact-integer and C-`int` boundaries, normalization/write-back including failure paths, `isdst` tri-state behavior under a restored controlled timezone, epoch `-1`, extreme times, result arity, and subtype. Run focused and full verification and keep JVM calendar/range substitutions explicit. | One coherent OS time-table commit with tests and gap-record update. |
