...
| --- | --- | --- | --- |
| 1 | M20 table sequence-mutation campaign | Audit `ltablib.c:aux_getn`, `tinsert`, and `tremove` plus the API get/set/length operations they use. Cover empty and sparse sequences, default and explicit positions, insertion/removal shifts, full-width extreme indices and lengths, table-like metatable routing, mutation/error partial effects, numeric-string coercion, ignored extras, result shape, and source-ordered diagnostics. Exit with any fixes, focused ordinary/table-like matrices, gap update, and full verification. | One coherent table-sequence commit with fixes, tests, and gap-record update. |
