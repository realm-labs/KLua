...
| --- | --- | --- | --- |
| 1 | M20 `string.pack`/`unpack` integer-layout campaign | Audit `lstrlib.c:getdetails`, `packint`, `copywithendian`, and the integer/alignment branches of `str_pack`, `str_unpack`, and `str_packsize`. Cover every fixed and sized integer option, sizes 1 through 16, native/opposite endian order, signed extension and overflow, zero padding, alignment caps, `X` look-ahead, initial-position behavior, ignored extras, result arity, and source-ordered diagnostics. Exit with any fixes, a focused cross-entry-point matrix, gap update, and full verification. | One coherent integer-layout commit with tests and gap-record update. |
