...
| --- | --- | --- | --- |
| 1 | M20 OS clock/exit audit | Audit `loslib.c:os_clock` and `os_exit` for clock origin/monotonic observations, boolean and integer status conversion, default status, close-flag truthiness and ordering, ignored extras, result arity, and embedder-safe exit propagation. Keep process termination test-owned through the configured exit handler, then run focused and full verification. | One coherent OS lifecycle commit with tests and explicit JVM clock/exit substitutions. |
