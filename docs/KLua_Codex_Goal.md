...
| --- | --- | --- | --- | --- |
| 1 | In progress | M20 VM tag-method chain boundary and tracer stack-safety | Match `lvm.c:luaV_finishget`, `luaV_finishset`, and `MAXTAGLOOP` for table, primitive, userdata, mixed-receiver, raw-hit, callable, and terminal-table `__index`/`__newindex` chains. Deep valid and rejected chains must not overflow KLua's lifecycle reachability traversal. Focused VM/stdlib tests and `./gradlew test` pass; update the gap note only if a residual remains. | One coherent VM, lifecycle-support, and regression-test commit. |
| 2 | Next | M20 byte-string fidelity closure audit | Against `lstrlib.c`, `lutf8lib.c`, core string construction, and conversion helpers, enumerate the remaining observable paths that still depend on JVM character semantics. Add a bounded differential case matrix for NUL, high bytes, malformed UTF-8, indexing, concatenation, conversion, and API round trips. Exit with the exact semantic and storage requirements for the byte-oriented string package; do not begin an unbounded representation rewrite inside the audit. | One focused test/audit and requirements commit, plus at most one independently reviewable correctness fix if the audit proves it safe. |
| 3 | Next | M19 continuation: canonical baseline and hot-model design | Establish the JDK 17 pre-refactor baseline and profile boxed values, table storage, call boundaries, debug/budget checks, coroutine transfer, and lifecycle tracing. Define internal migration seams and exit criteria for the first value/storage package without changing public or serialized APIs. | One benchmark/design commit; no broad representation rewrite in the baseline package. |

When package 1 closes, promote package 2 and select at most one new successor. After package 3 closes, select the next package from `Pre-v1 High-Performance Interpreter Track`. Do not keep closed campaign narratives or commit hashes in this table.
