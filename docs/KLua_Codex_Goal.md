...
| --- | --- | --- | --- |
| 1 | M19 small stack-slice snapshots | Preserve snapshot semantics while specializing zero- and one-value stack slices so common returns and native calls do not allocate a general `ArrayList` plus backing array. Preserve debug-return ordering and multi-result independence. Exit with focused snapshot/mutation tests, matched Lua-call/host/multi-return GC controls, and the full suite, or reject if JVM singleton lists do not provide a clear allocation win. | One focused VM collection/evidence commit; do not introduce lazy views over live frames. |
| 2 | M19 post-slice selection | Rerun the relevant controls and full suite, then select the next maintainable runtime cost from measured evidence. | One bounded profile/optimization package; do not mix unrelated workload families. |
