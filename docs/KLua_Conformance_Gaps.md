...
- KLua strings are still JVM-string-backed rather than byte-string-faithful in all standard-library paths. This leaves Lua 5.5 byte-level behavior incomplete for broader byte-sensitive standard-library cases. The raw-byte `utf8.offset`, `utf8.len`, and `utf8.codepoint` paths are source-backed for representable malformed layouts; the remaining UTF-8-specific uncertainty is the `utf8.codes` iterator plus general byte-string fidelity.
