...
- `load` and `loadfile` support table environments for loaded chunks. Non-table `_ENV` values are not fully modeled yet because KLua VM frames currently execute global reads and writes against a table environment.
