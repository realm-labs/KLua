...
- `debug.getinfo(..., "n")` resolves common source call-site names for local, global, upvalue, field, integer-indexed field, method, `__call`, `__index`, and `__newindex` calls, but broader Lua 5.5 name inference for indirect call patterns is not complete yet.
