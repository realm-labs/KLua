...
- Optional thread arguments for `debug.traceback`, `debug.getinfo`, `debug.getlocal`, `debug.setlocal`, `debug.sethook`, and `debug.gethook` are implemented for suspended KLua coroutine threads and empty-stack coroutine states such as dead coroutines. Broader cross-thread debug behavior beyond these KLua coroutine snapshots is not implemented yet.
...
- `string.dump` and binary `load` can round-trip Lua functions through KLua's internal bytecode package format using a JVM-string carrier, and the `strip` flag removes KLua debug metadata from dumped packages. Binary `loadfile` and general Lua byte-string fidelity remain incomplete, so dumped chunks are not yet portable Lua byte strings.
