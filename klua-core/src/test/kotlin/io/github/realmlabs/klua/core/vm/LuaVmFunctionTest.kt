...
class LuaVmFunctionTest {
    ...
    @Test
    fun `executes parenthesized indexed function call statements`() {
        val result = LuaVm().execute(
            Compiler.compile(
                """
                local total = 0
    ...
    @Test
    fun `goto out of loop scope closes captured locals before register reuse`() {
        val result = LuaVm().execute(
            Compiler.compile(
                """
                local function outer()
    ...
    fun `rejects calls to non function values`() {
        ...
        assertEquals("attempt to call a number value (local 'value')", error.message)
    }
}
